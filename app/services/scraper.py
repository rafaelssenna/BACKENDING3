                        ...
                        try:
                            ...
                            for lead in leads:
                                ...
                                if ph not in seen:
                                    ...
                                    log.info(f"✅ New lead [{total_yield}/{target}]: {nm or '(sem nome)'} — {ph}")
                            ...
                            if new == 0:
                                log.warning(f"⚠️ Page {idx} returned 0 new leads (empty_pages: {empty_pages}/{empty_limit})")
                            else:
                                log.info(f"📊 Page {idx}: Found {new} new leads (total: {total_yield}/{target})")
